    ...
    const DisplayPlayerInfo = async () => {
        ...
        if(a == "error"){
            ...
            for (s in d_data){
                ...
                d_data[s].xp = new Intl.NumberFormat().format(d_data[s].xp);
                d_data[s].xpLeft = new Intl.NumberFormat().format(d_data[s].xpLeft);
                d_data[s].rank = new Intl.NumberFormat().format(d_data[s].rank);
                const skillName = s;
                
                const capitalizedSkillName = skillName.charAt(0).toUpperCase()+ skillName.slice(1)
                const tempObj = d_data[s];
                s = capitalizedSkillName;
                d_data[capitalizedSkillName] = tempObj;
            }

            playerDetails = d_data;
